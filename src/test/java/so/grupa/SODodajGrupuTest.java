...
class SODodajGrupuTest {
	...
	public void testNeuspesnaValidacijaDrugaKlasa() {
		assertThrows(Exception.class, () -> sododajGrupu.execute(new Administrator()));
	}
	@Test
	public void testNeuspesnaValidacijaNull() {
		assertThrows(Exception.class, () -> sododajGrupu.execute(null));
	}
	@Test
	public void testNeuspesnaValidacijabrojClanovaVeciODKapaciteta() {
		Grupa g = new Grupa();
		SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy");
		Date d = null;
		try {
			d = sdf.parse("3.10.2100");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		g.setDatumPolaska(d);
		g.setKapacitetGrupe(15);
		g.setPrevoz("Bus1");
		...
		g.setClanovi(clanovi);
			...
			assertThrows(Exception.class, () -> sododajGrupu.validate(g));
	}

	
	@Test
	public void testNeuspesnaValidacijabrojClanovaManjiOD2() {
		Grupa g = new Grupa();
		SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy");
		Date d = null;
		try {
	...
	public void testUspesnoDodataGrupa() {
		...
		System.out.println("Grupe"+grupe.toString());
	...
	private void obrisiDodatogPutnikaIzBaze(Putnik pu) {
		...
		for (Putnik putnik : putnici) {
			if (pu.getEmail().equals(putnik.getEmail()) && pu.getBrojTelefona().equals(putnik.getBrojTelefona())) {
	...
	private Long vratiIDPutnikaIzBaze(Putnik p1) {
		ArrayList<Putnik> putnici = vratiSvePutnikeIzBaze();
		for (Putnik putnik : putnici) {
			if (p1.getEmail().equals(putnik.getEmail()) && p1.getBrojTelefona().equals(putnik.getBrojTelefona())) {
